...
def seq_collate(data):
    # trajectory data: (50, 23, 2), goals: (50,)
    ...
    obs_goals = torch.stack(obs_goals, dim=1)
    pred_goals = torch.stack(pred_goals, dim=1)
...
class SoccerDataset(Dataset):
    """Dataloder for the soccer trajectories datasets"""
    ...
    def __init__(self, path, n_agents, obs_len=10, pred_len=40):
        ...
        # 'trajectories' shape (seq_len, n_obs*n_agents, 2)
        # 'goals' shape (seq_len, n_obs)
    ...
    def __getitem__(self, idx):
        ...
        out = [
            ...
            self.obs_goals[:, idx], self.pred_goals[:, idx]
        ]
        return out
...
def data_loader(args, dset_path, set, shuffle=True):
    ...
    n_agents = 23 if (args.players == 'all') else 11
